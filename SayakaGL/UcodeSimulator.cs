...
namespace SharpOcarina.SayakaGL
{
    ...
    public static class UcodeSimulator
    {
        ...
        private static int LoadTexture(int ActiveTexture)
        {
            ...
            // Some imported OoT room display lists retain 0x02 texture pointers
            // even though the extracted texture bytes live in segment 0x03. Keep
            // the original scene address whenever it is valid; only fall back to
            // the room segment when the original pointer cannot be resolved.
